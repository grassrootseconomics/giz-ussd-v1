...
def test_transaction_balances_callback(activated_account,
    ...
    recipient_transaction['decimals'] = transaction_result.get("destination_token_decimals")
    sender_transaction['decimals'] = transaction_result.get("source_token_decimals")
    with pytest.raises(ValueError) as error:
